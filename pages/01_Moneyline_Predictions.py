...
csv_path = os.path.join(DATA_DIR, "nfl_gamelogs_vegas_2015-2025_ML_week15_copy.csv")
...
week16_teams = [
    ...
    {"Home": "Commanders", "Away": "Eagles"},
    {"Home": "Bears", "Away": "Packers"},
    {"Home": "Cowboys", "Away": "Chargers"},
    {"Home": "Giants", "Away": "Vikings"},
    {"Home": "Browns", "Away": "Bills"},
    {"Home": "Saints", "Away": "Jets"},
    {"Home": "Panthers", "Away": "Buccaneers"},
    {"Home": "Dolphins", "Away": "Bengals"},
    {"Home": "Titans", "Away": "Chiefs"},
    {"Home": "Broncos", "Away": "Jaguars"},
    {"Home": "Cardinals", "Away": "Falcons"},
    {"Home": "Lions", "Away": "Steelers"},
    {"Home": "Texans", "Away": "Raiders"},
    {"Home": "Ravens", "Away": "Patriots"},
    {"Home": "Colts", "Away": "49ers"},
]

# Week x - FANDUEL

st.markdown("---")
st.subheader("Week 16 Predictions - FanDuel Lines")

week16_games_fd = [
    get_team_features(df, 2025, "SEA", spread=+1.5, total=42.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "RAM", spread=-1.5, total=42.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "WAS", spread=+6.5, total=44.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "PHI", spread=-6.5, total=44.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CHI", spread=+1.5, total=46.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "GNB", spread=-1.5, total=46.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "DAL", spread=-2.5, total=49.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "SDG", spread=+2.5, total=49.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NYG", spread=+2.5, total=43.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "MIN", spread=-2.5, total=43.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CLE", spread=+10.5, total=41.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "BUF", spread=-10.5, total=41.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NOR", spread=-4.5, total=40.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NYJ", spread=+4.5, total=40.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CAR", spread=+3.0, total=45.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "TAM", spread=-3.0, total=45.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "MIA", spread=+4.5, total=47.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CIN", spread=-4.5, total=47.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "OTI", spread=+3.0, total=37.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "KAN", spread=-3.0, total=37.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "DEN", spread=-3.0, total=46.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "JAX", spread=+3.0, total=46.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CRD", spread=+2.5, total=47.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "ATL", spread=-2.5, total=47.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "DET", spread=-6.5, total=51.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "PIT", spread=+6.5, total=51.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "HTX", spread=-14.5, total=37.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "RAI", spread=+14.5, total=37.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "RAV", spread=-2.5, total=47.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NWE", spread=+2.5, total=47.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CLT", spread=+6.5, total=46.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "SFO", spread=-6.5, total=46.5, home=0, stat_cols_for_avg=stat_cols),
]

week16_df_fd = pd.DataFrame(week16_games_fd)

if st.button("Run Week 16 Predictions - FanDuel"):
    probs = model.predict_proba(week16_df_fd[features_avg])[:, 1]
    preds = (probs >= 0.5).astype(int)

    results_fd = []
    for i in range(0, len(probs), 2):  # pair home/away
        game_index = i // 2
        home = week16_teams[game_index]["Home"]
        away = week16_teams[game_index]["Away"]
...
# Week x - DRAFTKINGS
st.markdown("---")
st.subheader("Week 16 Predictions - DraftKings Lines")
...
week16_games_dk = [
    ...
    get_team_features(df, 2025, "DAL", spread=-2.5, total=49.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "SDG", spread=+2.5, total=49.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NYG", spread=+3.0, total=43.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "MIN", spread=-3.0, total=43.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CLE", spread=+10.0, total=42.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "BUF", spread=-10.0, total=42.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NOR", spread=-4.5, total=40.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "NYJ", spread=+4.5, total=40.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CAR", spread=+3.0, total=45.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "TAM", spread=-3.0, total=45.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "MIA", spread=+4.5, total=47.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CIN", spread=-4.5, total=47.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "OTI", spread=+3.5, total=37.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "KAN", spread=-3.5, total=37.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "DEN", spread=-3.0, total=46.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "JAX", spread=+3.0, total=46.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "CRD", spread=+3.0, total=48.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "ATL", spread=-3.0, total=48.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "DET", spread=-7.0, total=51.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "PIT", spread=+7.0, total=51.5, home=0, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "HTX", spread=-14.5, total=37.5, home=1, stat_cols_for_avg=stat_cols),
    get_team_features(df, 2025, "RAI", spread=+14.5, total=37.5, home=0, stat_cols_for_avg=stat_cols),
...
week16_df_dk = pd.DataFrame(week16_games_dk)

if st.button("Run Week 16 Predictions - DraftKings"):
    probs = model.predict_proba(week16_df_dk[features_avg])[:, 1]
    preds = (probs >= 0.5).astype(int)
